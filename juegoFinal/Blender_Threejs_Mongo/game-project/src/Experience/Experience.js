...
export default class Experience {
  ...
  resetGameToFirstLevel() {
    ...
    if (Array.isArray(this.world.enemies)) {
      this.world.enemies.forEach(e => e?.remove?.())
      this.world.enemies = []
    }
    ...
    // Revivir robot si está muerto
    if (!this.world.robot.body) {
      const shape = new CANNON.Sphere(0.4);
      this.world.robot.body = new CANNON.Body({
        mass: 2,
        shape: shape,
        position: new CANNON.Vec3(-17, 1.2, -67),
        linearDamping: 0.05,
        angularDamping: 0.9
      });
      this.world.robot.body.angularFactor.set(0, 1, 0);
      this.world.robot.body.material = this.physics.robotMaterial;
      this.physics.world.addBody(this.world.robot.body);
    }
